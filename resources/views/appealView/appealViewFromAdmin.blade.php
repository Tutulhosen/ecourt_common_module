@php
   ...
   $badi='ধারকের বিবরণ';
   $bibadi='খাতকের বিবরণ';
   switch ($results['appeal']['applicant_type']) {
        case 'BANK':
            $org_type = 'ব্যাংক';
            break;
        case 'GOVERNMENT':
...
<div class="card card-custom" style="margin-bottom: 20px">
   <div class="card-header flex-wrap py-5">
      {{-- <div class="card-title"> --}}
          <div class="container">
              <div class="row">
                  <div class="col-10"><h3 class="card-title h2 font-weight-bolder">{{ $results['page_title'] }}</h3></div>
      ...
      <div class="card-toolbar">
         <a href="{{ route('case.edit', $results['appeal']->id) }}" class="btn btn-sm btn-primary font-weight-bolder">
    ...
    <div class="row">
            ...
            <table class="table table-striped border">
               ...
               <tbody>
                   <tr>
                     ...
                     <td >{{ en2bn($results['appeal']['loan_amount']) ?? '-'}} টাকা</td>
                  </tr>
                  <tr>
                     <th scope="row">দাবিকৃত অর্থের পরিমাণ (কথায়)</th>
                     <td >{{ $results['appeal']['loan_amount_text'] ?? '-'}} টাকা</td>
                  </tr>
                 <tr>
                     <th scope="row">জেলা</th>
                     <td >{{ $results['appeal']['district_name'] ?? '-'}}</td>
                  </tr>
                  <tr>
                     <th scope="row">উপজেলা</th>
                     <td >{{ $results['appeal']['upazila_name']  ?? '-'}}</td>
                  </tr>
                  <tr>
                     <th scope="row">প্রাতিষ্ঠানের নাম</th>
                     <td >{{ $results['appeal']['office_name'] ?? '-'}}</td>
                  </tr>
                  <tr>
                     <th scope="row">প্রাতিষ্ঠানের ঠিকানা</th>
                     <td >{{get_office_by_id($results['appeal']['office_id'])->organization_physical_address ?? '-' }}</td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                     ...
                     <td >{{ get_office_by_id($results['appeal']['office_id'])->organization_routing_id  ?? '-' }}</td>
                  </tr>

                   <tr>
                     <th scope="row">মামলার ধারা</th>
                     <td >{{ $results['appeal']['law_section'] ?? '-'}}</td>
                  </tr>
                  <tr>
                     <th scope="row">মামলা নং</th>
                     <td >{{ en2bn($results['appeal']['case_no']) ?? '-'}}</td>
                  </tr>
                  <tr>
                     <th scope="row">মামলা রুজুর তারিখ</th>
                     <td >{{ en2bn($results['appeal']['case_date']) ?? '-'}}</td>
                  </tr>
      ...
      <div class="col-md-6">
            ...
            <table class="table table-striped border">
               ...
               <tbody>
              @php $k = 1; @endphp
         @foreach ($results['applicantCitizen'] as $badi)
            <tr style="text-align:center">
               <td>{{en2bn($k)}}.</td>
         ...
         @endforeach
            ...
            <br>
            @if(count($results['nomineeCitizen'])>0)
            <table class="table table-striped border">
               <thead>
               ...
               <tbody>
              @php $k = 1; @endphp
                @foreach ($results['nomineeCitizen'] as $nominee)
                <tr>
                   <td>{{en2bn($k)}}.</td>
      ...
      <div class="col-md-12">
         <table class="table table-striped border">
           ...
           <tbody>
              <tr>
                 <td>
                    @forelse ($results['attachmentList'] as $key => $row)
                          <div class="form-group mb-2" id="deleteFile{{ $row['id'] }}">
                              <div class="input-group">
                                  ...
                                  <div class="input-group-append">
                                      ...
                                      <a href="<?=getGccBaseUrl().'/'.$row['file_path'] . $row['file_name']?>" target="_blank" class="btn btn-sm btn-success font-size-h5 float-left">
        ...
        @if($results['appeal']['case_entry_type']=='RUNNING')
        <table class="table table-striped border">
            <thead>
