...
<div class="card card-custom" style="margin-bottom: 20px">
   <div class="card-header flex-wrap py-5">
      ...
      <div class="card-toolbar">
         <a href="{{ route('emc.news.create') }}" class="btn btn-sm btn-success font-weight-bolder">
                  ...
                  <div class="clearfix">
                       <div class="row">
                           <table class="table table-hover mb-6 font-size-h6">
                              <thead class="thead-success text-center" style="background-color: #008841; color:white">
                                 <tr>
                                    <h3>খবরের তালিকা</h3>
                              ...
                              <tbody class=" text-center">
                                 @foreach($data['short_news'] as $key => $row)
                                       @php
                                          $currentStatus = '';
                                          if($row->status == 1){
                                       ...
                                       <tr class="text-center">
                                          ...
                                          @endif
                                          <td width="300">
                                             <a href="{{url('emc/news/edit/'.$row->id)}}"style="color: white !important;">
                                                <button type="button" class="btn-sm btn btn-success">সম্পাদনা</button>
                                             </a>
                                                @if($row->status==1)
                                                   <a href="{{url('emc/news/status/0/'.$row->id)}}"style="color: white !important;">
                                                      <button type="button" class="btn-sm btn btn-warning">অপসারণ</button>
                                                   </a>
                                                @elseif($row->status==0)
                                                   <a href="{{url('emc/news/status/1/'.$row->id)}}"style="color: white !important;">
                                                      <button type="button" class="btn-sm btn btn-primary">পাবলিশ </button>
                                                   </a>
                                                @endif
                                             <a href="{{url('emc/news/delete/'.$row->id)}}" style="color: white !important;"><button type="button" class="btn-sm btn btn-danger"> মুছে ফেলুন</button>
                                             </a>
                                          </td>
                  ...
                  <div class="clearfix">
                       <div class="row">
                           <table class="table table-hover mb-6 font-size-h6">
                              ...
                              <tbody class=" text-center">
                                 @foreach($data['big_news'] as $key => $row)
                                       @php
                                          $currentStatus = '';
                                          if($row->status == 1){
                                             $currentStatus = 'প্রকাশিত হয়েছে'; 
