@php
// dd($data);
if(!empty($data->log_details_single_by_id->applicant))
{

    $applicant = json_decode($data->log_details_single_by_id->applicant);
}
else {
    $applicant=null;
}
if(!empty($data->log_details_single_by_id->victim))
{

    $victim = json_decode($data->log_details_single_by_id->victim);
}
else {
    $victim=null;
}
if(!empty($data->log_details_single_by_id->defaulter))
{

    $defaulter = json_decode($data->log_details_single_by_id->defaulter);
}
else {
    $defaulter=null;
}
if(!empty($data->log_details_single_by_id->witness))
{

    $witness = json_decode($data->log_details_single_by_id->witness);
}
else {
    $witness=null;
}
if(!empty($data->log_details_single_by_id->lawyer))
{

    $lawyer = json_decode($data->log_details_single_by_id->lawyer);
}
else {
    $lawyer=null;
}

if(!empty($data->log_details_single_by_id->defaulter_witness))
{

    $defaulter_witness = json_decode($data->log_details_single_by_id->defaulter_witness);
}
else {
    $defaulter_witness=null;
}

if(!empty($data->log_details_single_by_id->defaulter_lawyer ))
{

    $defaulter_lawyer  = json_decode($data->log_details_single_by_id->defaulter_lawyer );
}
else {
...
$case_date_from_created=explode(' ',$data->log_details_single_by_id->created_at)[0];

if($data->log_details_single_by_id->case_basic_info)
{
    $case_basic_info=json_decode($data->log_details_single_by_id->case_basic_info);
...
$files=json_decode($data->log_details_single_by_id->files);
                        ...
                        <tbody>
                            <tr>
                                <td>বিগত ইং {{ en2bn($case_basic_info->caseDate) }} তারিখ মোতাবেক বাংলা
                                    {{ /* BnSal( */$data->log_details_single_by_id->created_at, 'Asia/Dhaka', 'j F Y'/* ) */ }} সময়:
                                    @if (date('a', strtotime($data->log_details_single_by_id->created_at)) == 'pm')
                                        বিকাল
                                    @else
                                        সকাল
                                    @endif

                                    {{ en2bn(date('h:i:s', strtotime($data->log_details_single_by_id->created_at))) }}
                                ...
                                @foreach ($files as $key => $row)
                                          ...
                                          <div class="input-group">
                                              ...
                                              <div class="input-group-append">
                                                  {{-- <a href="{{ getEmcBaseUrl()}}/{{asset($row->file_path . $row->file_name) }}" target="_blank" class="btn btn-sm btn-success font-size-h5 float-left">
                                                      <i class="fa fas fa-file-pdf"></i>
                                                      <b>দেখুন</b>
                                                     
                                                   </a> --}}
                                                   <a href="{{ getEmcBaseUrl() }}/{{ $row->file_path . $row->file_name }}" target="_blank" class="btn btn-sm btn-success font-size-h5 float-left">
                                                    <i class="fa fas fa-file-pdf"></i>
                                                    <b>দেখুন</b>
                                                </a>
                                              </div>
