@php
    ...
    if (!empty($data->log_details_single_by_id->nominees)) {
        $nominees = json_decode($data->log_details_single_by_id->nominees);
    } else {
        $nominees = null;
    }
    ...
    //         ->where('gcc_appeals.id', $data->log_details_single_by_id->appeal_id)
    ...
    <div class="card card-custom">
        ...
        <div class="card-body">
            ...
            <div class="row" id="element-to-print">
                @if (!empty($data->case_basic_info))
                    <div class="col-md-12">
                        <p>আবেদনের তারিখ , {{ $data->case_basic_info->caseDate }}</p>
                        <p>মোট দাবী , {{ $data->case_basic_info->totalLoanAmount }}</p>
                        <p>মোট দাবী (কথায়) , {{ $data->case_basic_info->totalLoanAmountText }}</p>
                        <p>প্রাতিষ্ঠানের নাম , {{ $data->case_basic_info->applicant_office_name }} </p>
                        <p>প্রাতিষ্ঠানের ঠিকানা , {{ $data->case_basic_info->organization_physical_address }} </p>
                        <p>প্রাতিষ্ঠানের রাউটিং , {{ $data->case_basic_info->organization_routing_id }} </p>
                        <p>বিভাগ , {{ $data->appeal_data_from_gcc_appeal->division_name }}</p>
                        <p>জেলা , {{ $data->appeal_data_from_gcc_appeal->district_name }}</p>
                        <p>উপজেলা , {{ $data->appeal_data_from_gcc_appeal->upazila_name }}</p>
                        <p>কোর্ট , {{ $data->appeal_data_from_gcc_appeal->court_name }}</p>
                    </div>
                @endif
                <div class="col-md-12">
                    @if (!empty($defaulter))
                                ...
                                @foreach ($files as $key => $row)
                                    <div class="input-group">
                                        ...
                                        <div class="input-group-append">
                                            <a href="<?=getGccBaseUrl() . '/' . $row->file_path . $row->file_name?>" target="_blank"
