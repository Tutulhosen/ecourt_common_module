...
class GccAppealRepository
{
    ...
    public static function storeAppealBYCitizen($appealInfo)
    {
        ...
        $data['appeal_status'] = $appealInfo->status;
        $data['appealId'] = $appealInfo->appealId;
        $data['case_date'] = date('Y-m-d', strtotime(str_replace('/', '-', $caseDate)));
        $data['case_entry_type'] = $appealInfo->caseEntryType;
        $data['law_section'] = $appealInfo->lawSection;
        $data['applicant_type'] = $appealInfo->applicant_organization['Type'][0];
        $data['updated_at'] = date('Y-m-d H:i:s');
        $data['updated_by'] = $userIn->id;
        $data['flag_on_trial'] = 0;
        $data['office_id'] = $userIn->office_id;
        $data['office_name'] = $officeInfo->office_name_bn;
        $data['upazila_bbs_code'] = $officeInfo->upazila_id;
        $data['upazila_id'] = $officeInfo->upazila_id;
        $data['upazila_name'] = $officeInfo->upazila_name_bn;
        $data['district_bbs_code'] = $officeInfo->district_id;
        $data['district_id'] = $officeInfo->district_id;
        $data['district_name'] = $officeInfo->district_name_bn;
        $data['court_id'] = $appealInfo->court_id;
        $data['division_bbs_code'] = isset($officeInfo->division_bbs_code) ? $officeInfo->division_bbs_code : null;
        $data['division_id'] = $officeInfo->division_id;
        $data['division_name'] = $officeInfo->division_name_bn;
        $data['loan_amount'] = bn2en($appealInfo->totalLoanAmount);
        $data['loan_amount_text'] = $appealInfo->totalLoanAmountText;
        $data['office_unit_id'] = $userIn->citizen_type_id;
        $data['office_unit_name'] = 'প্রাতিষ্ঠানিক প্রতিনিধি';
        $data['created_at'] = date('Y-m-d H:i:s');
        $data['interestRate']= $appealInfo->interestRate;
        $data['payment_id']= $appealInfo->payment_id;
        $data['created_by']= $userIn->id;
