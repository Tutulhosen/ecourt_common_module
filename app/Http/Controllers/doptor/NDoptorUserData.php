...
class NDoptorUserData extends Controller
{
    ...
    public function getdependentlayer($id){

        if ($id==3) {
            $doptor_office = DB::table('officelayer')
                ->where('level', $id)
                ->select('customlayerid', DB::raw('MIN(level) as level'), 'nameBn')
                ->whereNotNull('customlayerid')
                ->groupBy('customlayerid', 'nameBn')
                ->get();
            $data['level']=$id;
            $data['doptor_office']=$doptor_office;
            return json_encode($data);
        }

        if ($id==4 || $id==5 || $id==6 || $id==7) {
            $doptor_office = DB::table('officeorigin_new')
                ->where('layer_level', $id)
                ->get();
            $data['level']=$id;
            $data['doptor_office']=$doptor_office;
    ...
    {
        $data['custom_office_2nd_layer']=null;
        $data['custom_office_2nd_layer_doptor']=null;
        $data['custom_office_2nd_layer_data']=null;
        $data['custom_office_2nd_layer_doptor_data']=null;
        ...
        if (!empty($request->custom_office_layer)) {
            // dd($request->custom_office_layer);
            if ($request->custom_office_layer==1 || $request->custom_office_layer==2) {
                $query->where('doptor_offices.layer_level', $request->custom_office_layer );

            }
            if ($request->custom_office_layer==3) {
                if (empty($request->custom_office_2nd_layer)) {
                    $query->where('doptor_offices.layer_level', $request->custom_office_layer );
                    
                } else {
                    $query->where('doptor_offices.custom_layer_id', $request->custom_office_2nd_layer );
                    $data['custom_office_2nd_layer'] = $request->custom_office_2nd_layer;
                    $data['custom_office_2nd_layer_data'] = DB::table('officelayer')
                                                        ->where('level', $request->custom_office_layer)
                                                        ->select('customlayerid', DB::raw('MIN(level) as level'), 'nameBn')
                                                        ->whereNotNull('customlayerid')
                                                        ->groupBy('customlayerid', 'nameBn')
                                                        ->get();
            ...
            if ($request->custom_office_layer==4 || $request->custom_office_layer==5 || $request->custom_office_layer==6 || $request->custom_office_layer==7) {
                if (empty($request->custom_office_2nd_layer_doptor)) {
                    $query->where('doptor_offices.layer_level', $request->custom_office_layer );
                } else {
                    $query->where('doptor_offices.office_origin_id', $request->custom_office_2nd_layer_doptor );
                    $data['custom_office_2nd_layer_doptor'] = $request->custom_office_2nd_layer_doptor;
                    $data['custom_office_2nd_layer_doptor_data'] = DB::table('officeorigin_new')
                                                                    ->where('layer_level', $request->custom_office_layer)
                                                                    ->get();
                }
        ...
        {
            $query->where('doptor_offices.office_name_bng', 'like', '%' . $request->office_name . '%');
        }

        $all_office_bangladesh=$query->paginate(30);
        $data['divisions'] = DB::table('geo_divisions')->get();   
        $data['ministry'] = DB::table('ministry')->get();   
        $data['custom_office_layer'] = DB::table('custom_office_layer')->get();   
