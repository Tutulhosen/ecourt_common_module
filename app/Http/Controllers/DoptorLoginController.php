...
class DoptorLoginController extends Controller
{
        ...
        if ($response->status == 'success') {
            ...
            if (empty($username)) {
                ...
                if (Auth::attempt(['username' => $username->username, 'password' => 'THIS_IS_N_DOPTOR_USER_wM-zu+93Fh+bvn%T78=j*G62nWH-C'])) {
                    // Session::put('court_type_id', $id);
                    $role=Auth::user()->role_id;
                    // Session::regenerate();
                    if ($role==34) {
                        return redirect()->route('dashboard.index');
                    }else {
                        return redirect()->route('doptor.court.select');
                    }
    ...
    public function login_d(Request $request){
        if (Auth::attempt(['username' => '200000003907', 'password' => 'THIS_IS_N_DOPTOR_USER_wM-zu+93Fh+bvn%T78=j*G62nWH-C'])) {
           
            return redirect("http://localhost:7878/callback");
         }
    }
}
